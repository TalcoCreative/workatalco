...
serve(async (req) => {
  ...
  try {
    ...
    // 7. Seed default roles, permissions & national holidays for this company
    try {
    ...
    if (superAdminRole) {
      // Insert new assignment (unique constraint is now on user_id + role_id)
      await supabaseAdmin.from("user_dynamic_roles").insert({
        user_id: adminUserId,
